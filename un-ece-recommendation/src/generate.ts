...
for (const list of lists) {
	for (const version of versions) {
		const source = `https://www.xrepository.de/api/xrepository/urn:xoev-de:kosit:codeliste:rec${list}_${version}/download/UN_ECE_Recommendation_N_${list}_${version}.json`;
		...
		await fs.writeFile(
			...
			export const REC_${list}_${version}: CodeList = ${JSON.stringify(data)}
			`,
		);

		await fs.appendFile(
			`src/index.ts`,
			`export * from "./${list}/${version}/index.js";\n`,
		);
